...
const images = import.meta.glob('../../photos/Drone/*.{jpg,png}', { eager: true });
...
export default function Nature() {
  const photos = useMemo(() => {
    ...
    const stored = sessionStorage.getItem('drone-photos');
    if (stored) return JSON.parse(stored);

    // Shuffle once, then save to sessionStorage
    const shuffled = shuffleArray(allPhotos);
    sessionStorage.setItem('drone-photos', JSON.stringify(shuffled));
    return shuffled;
  }, []);
